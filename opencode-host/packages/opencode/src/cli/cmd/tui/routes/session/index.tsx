...
import { BoxRenderable, MouseButton, ScrollBoxRenderable, addDefaultParsers, TextAttributes, RGBA, type MouseEvent } from "@opentui/core"
...
const SIDEBAR_MAX_WIDTH = 72
const SIDEBAR_MIN_CONTENT_WIDTH = 64
const SIDEBAR_RESIZE_HANDLE_WIDTH = 1
const SIDEBAR_DEFAULT_RATIO = 0.3
...
function sidebarWidthBounds(terminalWidth: number) {
  const min = Math.min(SIDEBAR_MIN_WIDTH, Math.max(24, terminalWidth - SIDEBAR_MIN_CONTENT_WIDTH))
  const max = Math.max(min, Math.min(SIDEBAR_MAX_WIDTH, terminalWidth - SIDEBAR_MIN_CONTENT_WIDTH))
  return { min, max }
}
...
const sessionBindingCommands = [
  ...
  "session.sidebar.wider",
  "session.sidebar.reset_width",
...
type AnswerTreeScrollGlobal = typeof globalThis & {
  __answerTreeScrollToMessage?: (input: AnswerTreeScrollRequest) => void
}
...
export function Session() {
  ...
  const [sidebarWidthRatio, setSidebarWidthRatio] = kv.signal<number>("sidebar_width_ratio", SIDEBAR_DEFAULT_RATIO)
  ...
  const sidebarWidth = createMemo(() => {
    const ratio = Number.isFinite(sidebarWidthRatio()) ? sidebarWidthRatio() : SIDEBAR_DEFAULT_RATIO
    const width = Math.round(dimensions().width * ratio)
    const { min, max } = sidebarWidthBounds(dimensions().width)
    return clamp(width, min, max)
  })
  const resizeSidebar = (width: number) => {
    const { min, max } = sidebarWidthBounds(dimensions().width)
    const nextWidth = clamp(Math.round(width), min, max)
    setSidebarWidthRatio(() => nextWidth / dimensions().width)
  }
  const resizeSidebarFromMouse = (evt: MouseEvent) => {
    if (evt.button !== MouseButton.LEFT) return
    if (evt.type !== "down" && evt.type !== "drag") return
    evt.preventDefault()
    evt.stopPropagation()
    resizeSidebar(dimensions().width - evt.x - SIDEBAR_RESIZE_HANDLE_WIDTH)
  }
  ...
  const sidebarTotalWidth = createMemo(() =>
    sidebarVisible() ? sidebarWidth() + SIDEBAR_RESIZE_HANDLE_WIDTH : 0,
  )
  const contentWidth = createMemo(() => dimensions().width - sidebarTotalWidth() - 4)
  ...
  const scrollToAnswerTreeMessage = (input: AnswerTreeScrollRequest) => {
    if (input.sessionID !== route.sessionID) return
    const messageID = findAnswerTreeMessageID(input)
    if (!messageID || !scroll || scroll.isDestroyed) return
  ...
  ;(globalThis as AnswerTreeScrollGlobal).__answerTreeScrollToMessage = scrollToAnswerTreeMessage
  onCleanup(() => {
    const target = globalThis as AnswerTreeScrollGlobal
    if (target.__answerTreeScrollToMessage === scrollToAnswerTreeMessage) delete target.__answerTreeScrollToMessage
  })
  ...
  const sessionCommandList = createMemo(() => [
    ...
    },
    {
      ...
      value: "session.sidebar.reset_width",
      category: "Session",
      run: () => {
        setSidebarWidthRatio(() => SIDEBAR_DEFAULT_RATIO)
        dialog.clear()
      },
    },
    {
            ...
            <Switch>
              <Match when={wide()}>
                <box
                  ...
                  onMouse={resizeSidebarFromMouse}
                />
...
function ToolPart(props: { last: boolean; part: ToolPart; message: AssistantMessage }) {
  ...
  const shouldHide = createMemo(() => {
    if (props.part.tool.startsWith("answer_tree_") && props.part.state.status === "completed") return true
